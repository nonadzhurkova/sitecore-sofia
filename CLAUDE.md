...
- `/sitemap.xml` — generated by `app/sitemap.ts` (Next.js `MetadataRoute.Sitemap`); lists all static routes plus every event and blog post, sourced dynamically from `app/data/events.ts` and `getAllBlogPosts()`
- `/robots.txt` — generated by `app/robots.ts`, allows all crawlers and points to `/sitemap.xml`
...
- The production URL (`https://sitecore-sofia.vercel.app`) is hardcoded in `app/sitemap.ts` and `app/robots.ts` — sitemap entries always point there even when previewed on localhost
